...
async function fetchCourseDetailWithStatus(insId = 1, unitId) {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/fetch_instructor_details?insId=${insId}&unit_id=${unitId}&student_id=${studentId}`
    );
...
async function get_lessons(unitId) {
  try {
    const response = await fetch(`${API_BASE_URL}/fetch_lessons_and_students_2?unit_id=${unitId}`)
    const data = await response.json();
